...
export default function RatingModal({ ride, raterRole, targetName, targetPhoto, onClose }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!isDriver && ride.driver_id) {
        const { data: allRides, error } = await supabase.from("ride_requests").select("*").eq("driver_id", ride.driver_id);
        if (!error && allRides) {
          ...
          if (rated.length > 0) {
            ...
            await supabase.from("drivers").update({ rating: parseFloat(avg.toFixed(1)) }).eq("id", ride.driver_id);
          }
        }
      }
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      className="fixed inset-0 z-[200] bg-black/70 flex items-end" onClick={() => onClose(true)}>
      <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }}
        ...
        <div className="flex items-center justify-between">
          <h3 className="font-bold text-xl text-slate-900">{isDriver ? "Califica al pasajero" : "Califica al conductor"}</h3>
          <button onClick={() => onClose(true)} className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center">
        ...
        <div className="flex items-center gap-3">
          <div className="w-14 h-14 rounded-2xl overflow-hidden bg-slate-100 flex items-center justify-center flex-shrink-0">
            {targetPhoto ? <img src={targetPhoto} alt="" className="w-full h-full object-cover" /> : <span className="text-2xl font-black text-slate-400">{targetName?.charAt(0)}</span>}
          </div>
          <div>
            <p className="font-bold text-slate-900">{targetName}</p>
            <p className="text-xs text-slate-400">Servicio #{ride?.service_id || ride?.id?.slice(-6)}</p>
          </div>
        </div>
          ...
          <div className="flex items-center justify-center gap-2">
            {[1,2,3,4,5].map(s => (
              <button key={s} onMouseEnter={() => setHovered(s)} onMouseLeave={() => setHovered(0)}
                onTouchStart={() => setHovered(s)} onClick={() => { setStars(s); setHovered(0); }}
                className="p-1 transition-transform active:scale-90">
                <Star className={`w-10 h-10 transition-all ${s <= activeStars ? "fill-amber-400 text-amber-400 scale-110" : "text-slate-200"}`} />
              </button>
            ))}
          </div>
          {activeStars > 0 && <p className="text-center text-sm font-semibold text-amber-500">{labels[activeStars]}</p>}
        </div>
        <textarea value={comment} onChange={e => setComment(e.target.value)}
          placeholder="Agrega un comentario (opcional)..." rows={3}
        ...
        <div className="flex gap-3">
          <button onClick={() => onClose(true)} className="flex-1 py-3 text-sm text-slate-400 border border-slate-200 rounded-2xl font-medium">Omitir</button>
          <Button onClick={handleSubmit} disabled={stars === 0 || saving}
