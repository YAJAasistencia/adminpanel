...
export default function DriverHelpTicket({ driver, rideContext, onClose }) {
  ...
  const { data: tickets = [] } = useQuery({
    ...
    queryFn: async () => {
      try {
        const { data, error } = await supabase.from("support_tickets").select("*").eq("driver_id", driver.id).order("created_date", { ascending: false });
  ...
  const { data: driverRides = [] } = useQuery({
    ...
    queryFn: async () => {
      try {
        const { data, error } = await supabase.from("ride_requests").select("*").eq("driver_id", driver.id).order("created_date", { ascending: false });
        if (error) throw error;
        return data || [];
      } catch (err) {
