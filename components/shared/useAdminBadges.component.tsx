 ...
 *   passengerUserId: string (if passenger)
 *   passengerName: string (if passenger)
 *   driverName: string (if driver)
 *   passengerPhone: string (if passenger) — used to load ride history
 *   rideContext: { ride_id, passenger_name, service_id } — pre-fill for a specific ride
 *   onClose: () => void
 *   darkMode: boolean — use dark background (for driver/passenger apps)
 */
export default function TicketsPanel({
  role, driverId, passengerUserId, passengerName, driverName, passengerPhone,
  rideContext, onClose, darkMode = true
}) {
  // If rideContext provided, open directly in "new"
  ...
  // Load past rides for ride selector (passenger only, when no rideContext)
  ...
  // Load past rides for driver selector (driver only, when no rideContext)
