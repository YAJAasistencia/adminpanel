...
export default function RatingModal({ ride, raterRole, targetName, targetPhoto, onClose }) {
  ...
  const handleSubmit = async () => {
    ...
    await base44.entities.RideRequest.update(ride.id, updates);

    // If passenger rates the driver, update driver's average rating
    if (!isDriver && ride.driver_id) {
      const allRides = await base44.entities.RideRequest.filter({ driver_id: ride.driver_id });
      ...
      if (rated.length > 0) {
        ...
        await base44.entities.Driver.update(ride.driver_id, { rating: parseFloat(avg.toFixed(1)) });
      }
    }
