...
/**
 * PATCH /api/support-tickets?id=xxx
 * Actualizar registro
 */
export async function PATCH(request: Request) {
  try {
    ...
    const body = await request.json();
    const result = await supportTicketService.update(id, body);
...
/**
 * DELETE /api/support-tickets?id=xxx
 * Eliminar registro
 */
export async function DELETE(request: Request) {
  try {
    ...
    const result = await supportTicketService.delete(id);
    if (!result.success) return NextResponse.json({ error: result.error }, { status: 500 });
    return NextResponse.json({ data: { success: true }, success: true });
  } catch (error) {
    console.error('DELETE /api/support-tickets error:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
